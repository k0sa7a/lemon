...
export default class extends Controller {
  ...
  update(e) {
    ...
    if (
      this.itemTarget.classList.value.includes("itinerary_table__start-point")
    ) {
      deleteStart = true;
    }
  ...
  setStart(event) {
    ...
    fetch(url, {
      ...
      .then(() => {
        this.itineraryShowController.setStartCoords();
      })
      .then(() => {
        if (startRemoved == false) {
          document.querySelector(".truck").remove();
        }
      })
      .then(() => {
  ...
  async resetStart() {
    ...
    await fetch(url, {
      ...
      .then(() => {
        return console.log("complete resetStart");
      })
      .catch((err) => console.log(err))
      .then(() => {
        return console.log("returned reset");
      });
  }
